...
// --- カーソル直前の「最後のフレーズ」を取得（カンマ/改行/日本語カンマ区切り） ---
...
function showSuggestionsFor(kind, editorEl) {
  ...
  // 部分一致（includes）＋ 前方一致（startsWith）優先ソート
...
function applySuggestion(currentToken, suggestion) {
  ...
  // 続けて入力したいはずなのでそのまま次の候補も出す
  showSuggestionsFor(kind, editor);
}

// --- 4つのテキストエリアにサジェスト用のイベントをまとめて付ける ---
function attachSuggestHandlers(editorEl, kind) {
  if (!editorEl) return;
...
attachSuggestHandlers(editorNegEl, "neg");
attachSuggestHandlers(presetEditorPosEl, "presetPos");
attachSuggestHandlers(presetEditorNegEl, "presetNeg");
...
function renderWordsEditList() {
  ...
  multiLabel.style.marginLeft = "8px";
  ...
  // の処理をそのまま使ってOKです（draggingまわりだけ）
    ...
    chip.addEventListener("touchmove", (e) => {
      ...
      if (!dragInfoWordEdit.dragging && dist > 5) {
        ...
        // 強調/弱体モードのときは色を変えない（draggingクラスを付けない）
        if (wordsEditMode !== "weight") {
          dragInfoWordEdit.chipEl.classList.add("dragging");
        }
      }
      if (dragInfoWordEdit.dragging && wordsEditMode !== "weight") {
        e.preventDefault();
      }
    }, { passive: false });
    ...
    chip.addEventListener("touchend", (e) => {
      ...
      // ★ 強調/弱体モード：ドラッグ中（スクロール）なら何もしない。タップのみ選択切り替え
      if (wordsEditMode === "weight") {
        if (info.dragging) {
          return; // スクロール扱い
        }
        if (wordsMultiSelectMode) {
          if (selectedWordIndices.has(info.index)) {
            selectedWordIndices.delete(info.index);
          } else {
            selectedWordIndices.add(info.index);
          }
          renderWordsEditList();
        }
        return;
    ...
    chip.addEventListener("mouseup", (e) => {
      ...
      // ★ 強調/弱体モード：ドラッグ中なら何もしない。クリックのみ選択切り替え
      if (wordsEditMode === "weight") {
        if (info.dragging) {
          return;
        }
